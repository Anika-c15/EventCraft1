...
const typeVariant = (type: ApprovalType) => {
  switch (type) {
...
export const Approvals: React.FC = () => {
  ...
  const pending = appApprovals.filter((a) => a.status === 'pending')
  const resolved = appApprovals.filter((a) => a.status !== 'pending')
        ...
        <div>
          ...
          <div className="space-y-3">
            {resolved.map((approval) => (
              <div
                key={approval.id}
                className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 opacity-80"
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span className="text-xs text-gray-400 flex-shrink-0 ml-4">
                    {approval.resolvedAt ? formatDate(approval.resolvedAt) : formatDate(approval.createdAt)}
                  </span>
                </div>
                ...
                {approval.resolvedBy && (
                  <p className="text-xs text-gray-400 mt-2">
                    Resolved by <span className="font-medium">{approval.resolvedBy}</span>
                  </p>
                )}
              </div>
            ))}
